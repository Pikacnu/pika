...
export const Timeline = ({ eradata }: EraProps) => {
  ...
  if(!eradata){
    eradata = [{eraname: 'Error',describe:'',thing: [{ date: new Date(), thing: 'undefind'}]}];
  }
      ...
      const things = thing.map((obj,index) => {
        ...
        <>
          <div className={styles.event} key={index}>
            <div className={styles.thing} key={`${index} something`}>
              <div className={styles.year} key='year'>{format(date, 'yyyy')}</div>
              <div className={styles.thing} key='thing'>{thing}</div>
            </div>
            {other ?.map((things,index)=>{return <div key={index} className={styles.other}>{`${things.replace(/\(/g, ' ⁀').replace(/\)/g, '‿')}`}</div>})}
          </div>
        </>
      )})
      return (
  ...
  return (
    <div className={`${styles.root} ${styletheme}`}>
      <button className={styles.btn} onClick={toggleTheme}>{!theme?(<img src='./sun.svg' alt='Light Theme'></img>):<img src='./moon.svg'  alt='Dark Theme'></img>}</button>
